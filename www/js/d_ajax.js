...
function contactAjax(url, method, params){
  ...
  xhr.onreadystatechange = function(){
    if(xhr.readyState == 4){
      var res = xhr.responseText;
      alert("Your Contact Has been Added, Check Contact and Proceed to Message");
      if(res){

        window.location = "u_message";
      }
      ////console.log(res);
    }
  }
...
function getPendingTaskCount(){
  ...
  var params = 'session='+ document.getElementById('session').value;
  // var params = 'state=' + document.getElementById('stateid').value;
  showPendingTaskCount(url,method,params);
  ////console.log(url);
}

function showPendingTaskCount(url, method,params){
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function(){
    if(xhr.readyState == 4){
      var res = xhr.responseText;
      // document.getElementById('ptaskNot').innerHTML = res
      document.getElementById('ptaskNot2').innerHTML = res
      //console.log(xhr.responseText);
    }
  }
  xhr.open(method, url, true);
...
function getTaskCount(){
  ...
  var params = 'session='+ document.getElementById('session').value;
  // var params = 'state=' + document.getElementById('stateid').value;
  showTaskCount(url,method,params);
  //console.log(url);
}

function showTaskCount(url, method,params){
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function(){
    if(xhr.readyState == 4){
      var res = xhr.responseText;
      // document.getElementById('taskNot').innerHTML = res
      document.getElementById('taskNot2').innerHTML = res
      //console.log(xhr.responseText);
    }
  }
  xhr.open(method, url, true);
